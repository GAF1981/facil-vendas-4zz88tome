...
interface BoletoImportDialogProps {
  onSuccess?: () => void
}

export function BoletoImportDialog({ onSuccess }: BoletoImportDialogProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      ...
      try {
        const data = await boletoService.parseCSV(selectedFile)
        setParsedData(data)
            ...
            <div className="space-y-4">
              <div className="bg-muted/50 p-4 rounded-lg border text-center">
                ...
                <p className="text-sm font-medium">
                  {parsedData.length} linhas processáveis encontradas
                </p>
              </div>

-- Fix for get_client_projections to correctly aggregate items per order
-- and robustly parse varied date string formats (including YYYY/MM/DD and DD-MM-YYYY)
-- Ensures Order 607 (and others) are properly grouped before ranking.
  ...
  parsed_data AS (
      SELECT
        ...
        CASE
            ...
            WHEN trim(date_str) ~ '^\d{2}/\d{2}/\d{2}$' THEN 

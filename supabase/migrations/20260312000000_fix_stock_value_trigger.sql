-- Migration to fix Stock Value Calculation and Synchronization
-- This ensures 'QUANTIDADE DE ESTOQUE FINAL' is correctly populated and kept in sync with 'BANCO_DE_DADOS'

-- 1. Ensure the column exists and has an index for performance
DO $$
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'QUANTIDADE DE ESTOQUE FINAL' AND column_name = 'VALOR ESTOQUE POR PRODUTO') THEN
        ALTER TABLE "QUANTIDADE DE ESTOQUE FINAL" ADD COLUMN "VALOR ESTOQUE POR PRODUTO" NUMERIC DEFAULT 0;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'QUANTIDADE DE ESTOQUE FINAL' AND column_name = 'VALOR ESTOQUE SALDO FINAL') THEN
        ALTER TABLE "QUANTIDADE DE ESTOQUE FINAL" ADD COLUMN "VALOR ESTOQUE SALDO FINAL" NUMERIC DEFAULT 0;
    END IF;
END $$;

-- 2. Create or Replace the Sync Function with Robust Calculation Logic
